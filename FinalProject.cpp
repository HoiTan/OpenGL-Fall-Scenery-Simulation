...
Display( )
{
	...
	gluLookAt( -50.f, 54.f, 53.f,     0.f, 5.f, 0.f,     0.f, 1.f, 0.f );
	...
	// ^ rotate – about X
	// L leaf
	// % random select one of the above

	// Tree1: F[>A][<A][+A][-A][vA][^A], deg:35.0f
	//F[+A][-A]<A>AvA^AL
	//F[>A][<A][+A][-AvA][^A]L
    ...
    for (const auto &leaf : leafPositions) {
		...
		// Create a rotation matrix from the leaf's orientation vectors
		// Assuming the leaf model extends along the positive Z-axis
		glm::mat3 orientationMatrix;
		orientationMatrix[0] = glm::normalize(leaf.right);  // X-axis
		orientationMatrix[1] = glm::normalize(leaf.up);     // Y-axis
		orientationMatrix[2] = glm::normalize(glm::cross(leaf.right, leaf.up)); // Z-axis
		
		// Convert glm::mat3 to glm::mat4
...
InitGraphics( )
{
    ...
    if( ! valid )
    {
        fprintf( stderr, "Yuch! The leaf shader did not compile.\n" );
    }
    else
    {
...
void replace(float* arr1, float* arr2){
	arr1[1] = arr2[1] + .02;
	arr1[0] = arr2[0];
}
...
InitLists( )
{
    ...
    #define NX	400			// how many points in x
    ...
    #define NZ	400         	// how many points in z
